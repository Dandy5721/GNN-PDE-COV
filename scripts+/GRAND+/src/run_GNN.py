...
def train_OGB(model, mp, optimizer, data, pos_encoding=None):
  ...
  x_train = (feat).cpu().numpy()
  io.savemat('/ram/USERS/bendan/NIPS/graph-neural-pde-main/'+dataset1+'/x_train_GRAND_ori_8.mat', {'data': x_train})
  out_fea_ori = out.cpu().detach().numpy()
  io.savemat('/ram/USERS/bendan/NIPS/graph-neural-pde-main/'+dataset1+'/out_train_ori_GRAND_ori_8.mat', {'data': out_fea_ori})
...
def test(model, data, pos_encoding=None, opt=None):  # opt required for runtime polymorphism
  ...
  io.savemat('/ram/USERS/bendan/NIPS/graph-neural-pde-main/'+dataset1+'/x_test_GRAND_ori_8.mat', {'data': x_train})
  out_fea_ours = logits.cpu().detach().numpy()
  io.savemat('/ram/USERS/bendan/NIPS/graph-neural-pde-main/'+dataset1+'/out_test_GRAND_ori_8.mat', {'data': out_fea_ours})
