...
parser.add_argument('--number_layer', type=int, default=128)
...
class DeeperGCN(torch.nn.Module):
    def __init__(self, hidden_channels, num_layers):
        ...
        self.lins = Linear(hidden_channels, hidden_channels)
...
model = DeeperGCN(hidden_channels=args.hidden_channels, num_layers=args.number_layer).to(device)
data = data.to(device)
print(model)
...
def train():
    ...
    # print(dataset)
    io.savemat(''+dataset1+'/x_train_ResGCN.mat', {'data': x_train})

    if args.GTV_number == 0:
        out_fea_ori = out_f.cpu().detach().numpy()
        io.savemat(''+dataset1+'/out_train_ori_ResGCN.mat', {'data': out_fea_ori})
  
    elif args.GTV_number == 2:
        out_fea_ours = out_f.cpu().detach().numpy()
        io.savemat(''+dataset1+'/out_train_ResGCN.mat', {'data': out_fea_ours})
...
def test():
    ...
    dataset1=args.dataset
    x_train = (data.x).cpu().numpy()
    io.savemat(''+dataset1+'/x_test_ResGCN.mat', {'data': x_train})

    if args.GTV_number == 0:
        out_fea_ori = out_f.cpu().detach().numpy()
        io.savemat(''+dataset1+'/out_test_ori_ResGCN.mat', {'data': out_fea_ori})
  
    elif args.GTV_number == 2:
        out_fea_ours = out_f.cpu().detach().numpy()
        io.savemat(''+dataset1+'/out_test_ResGCN.mat', {'data': out_fea_ours})
