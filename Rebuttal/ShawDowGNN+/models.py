...
class DeepGNN(nn.Module):
    ...
    def __init__(
        ...
        self.gtv = GTVBlock(self.dim_hid)
    ...
    def forward(
        ...
        for i in range(num_ensemble):
            ...
            for ci, md in enumerate(self.conv_layers[i]):
                xmd = md(xmd, sizes_subg=size_subg_ens[i])
                if ci == 0:
                    self.gtv.z = torch.zeros(xmd[0].shape[0]-1, xmd[0].shape[1]).to(xmd[0].device)
                xx = self.gtv(xmd[0])
                xmd = (xx, xmd[1], xmd[2], xmd[3])
                xjk.append(xmd[0])
            # residue and pooling
